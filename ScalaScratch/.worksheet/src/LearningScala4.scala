...
var numList = List(1);System.out.println("""numList  : List[Int] = """ + $show(numList ));$skip(43); 
for (x<-2 to 20) {
	numList = x :: numList
};$skip(27); 
println(numList.reverse);$skip(26); 
numList = numList.reverse;$skip(82); 


val resMap = numList.map( (i : Int) => if (i%3==0) i else None).filter(_!=None);System.out.println("""resMap  : List[Any] = """ + $show(resMap ));$skip(54); 

val resFilter = numList.filter( (x : Int) => x%3==0);System.out.println("""resFilter  : List[Int] = """ + $show(resFilter ))}
