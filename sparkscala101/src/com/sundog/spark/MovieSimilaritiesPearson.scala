...
object MovieSimilaritiesPearson {
  ...
  def computePearsonSimilarity(ratingPairs : RatingPairs) : (Double, Int) = {
    var numPairs : Int = 0
    ...
    var pearsonScore : Double = 0.0
    val numerator = sum_xy - (sum_x * sum_y/numPairs)
    val equation1 = sum_xx - ((sum_x * sum_x)/numPairs)
    val equation2 = sum_yy - ((sum_y * sum_y)/numPairs)
    val denominator = Math.sqrt(equation1 * equation2)

    if (denominator!=0){
      pearsonScore = numerator/denominator
    } 
    
    return (pearsonScore, numPairs) 
  ...
  def main(args: Array[String]) {
    ...
    val moviePairSimilarities = aggMoviePairRatings.mapValues(computePearsonSimilarity).cache()
    ...
    if (args.length > 0) {
    	val scoreThreshold = 0.80 // 0.97
    	val coOccurrenceThreshold = 5.0 //50.0
